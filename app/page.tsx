...
import { getHistory, saveHistory } from "@/lib/historyService";
import { getSavedConfig } from "@/lib/llmConfigService";
...
export default function Home() {
  ...
  // 启动时读取服务端配置、本机持久化配置与历史条数
  useEffect(() => {
    fetch("/api/config")
      .then((r) => r.json())
      .then((c) => {
        setEnvHasApiKey(!!c.hasApiKey);
        if (c.model) setLlmConfig((p) => ({ ...p, model: c.model }));
        if (c.baseUrl) setLlmConfig((p) => ({ ...p, baseUrl: c.baseUrl }));
      })
      .catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      try {
        saveHistory(meta.fileName, meta.fileType, finalItems, "default", false);
        setHistoryCount(getHistory().length);
      } catch {
        // 静默处理
      }
  ...
  const handleStartBatch = useCallback(async () => {
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      try {
        ...
        // 保存到历史记录
        try {
          saveHistory(meta.fileName, meta.fileType, finalItems, "default", true, [meta.fileName]);
          setHistoryCount(getHistory().length);
        } catch {
          // 静默处理
        }
  ...
  return (
    <div className="app-shell min-h-screen relative overflow-hidden">
      ...
      <PageHeader
        ...
        configReady={envHasApiKey || !!llmConfig.apiKey}
